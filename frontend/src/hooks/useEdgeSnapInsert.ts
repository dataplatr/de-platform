...
export function useEdgeSnapInsert(
  ...
  const {
    nodes, edges, batchUpdate, addNode, setSelectedNode, setRightPanelTab,
   ...
   */
  const snapOrphanNode = useCallback((nodeId: string, finalPos: { x: number; y: number }) => {
   ...
   */
  const onToolDrop = useCallback((e: React.DragEvent): boolean => {
    ...
    if (type === 'output') {
      ...
      addNode({
        id: makeNodeId(), type: 'output', label: `output_${count + 1}`,
        config: DEFAULT_CONFIGS['output'], position: pos,
      })
      return true
    }
    ...
    if (!best) {
      ...
      addNode({
        id: makeNodeId(), type, label: `${label} ${count + 1}`,
        config: DEFAULT_CONFIGS[type], position: dropPos,
      })
      return true
    }
    ...
    setRightPanelTab('config')
    return true
  }, [nodes, edges, rfInstance, batchUpdate, addNode, setImportPos, setSelectedNode, setRightPanelTab])
