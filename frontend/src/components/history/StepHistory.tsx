  ...
  if (nodes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2 text-muted p-6 text-center">
        ...
        <p className="text-[11px] text-muted">Drag a table onto the canvas to add the first step.</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-theme">
        ...
        <span className="text-[10px] text-muted">{edges.length} connection{edges.length !== 1 ? 's' : ''}</span>
      </div>

      {ordered.map((node, idx) => {
        const meta = NODE_META[node.type] ?? { icon: '●', labelClass: 'text-muted', label: node.type }
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className={clsx('text-[10px] font-semibold uppercase tracking-wide', meta.labelClass)}>
                  {meta.label}
                </span>
              </div>
