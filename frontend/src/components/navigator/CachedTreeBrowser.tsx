...
import {
  ChevronDown, ChevronRight, Database, Loader2, RefreshCw, Table2,
} from 'lucide-react'
...
function ColTypeBadge({ type }: { type: string }) {
  const colors: Record<string, string> = {
    VARCHAR: 'text-[#9cdcfe]', INTEGER: 'text-[#b5cea8]',
    FLOAT: 'text-[#b5cea8]', DATE: 'text-[#ce9178]',
    TIMESTAMP: 'text-[#ce9178]', BOOLEAN: 'text-[#569cd6]',
  }
  const abbr: Record<string, string> = {
    VARCHAR: 'str', INTEGER: 'int', FLOAT: 'flt',
    DATE: 'date', TIMESTAMP: 'ts', BOOLEAN: 'bool',
  }
  return (
...
export function CachedTreeBrowser({ connection, search = '' }: { connection: DatabricksConnection; search?: string }) {
  ...
  const load = useCallback((silent = false) => {
    ...
    return api.getCachedTables(connection.id)
      .then(({ data }) => {
  ...
  const syncSchema = useCallback((catalog: string, schema: string) => {
    const key = `${catalog}.${schema}`
    setSyncingSchemas(prev => new Set(prev).add(key))
    // Mark as pending in local state so polling kicks in
    setSelectedSchemas(prev =>
      prev.map(s => s.catalog === catalog && s.schema === schema
        ? { ...s, synced_at: null }
        : s
      )
    )
    ...
    setTimeout(() => {
      load(true).then(() => {
        setSyncingSchemas(prev => { const s = new Set(prev); s.delete(key); return s })
      })
    }, 3000)
  }, [connection.id, load])

  const toggleCatalog = (cat: string) =>
    setExpandedCatalogs(prev => { const s = new Set(prev); s.has(cat) ? s.delete(cat) : s.add(cat); return s })

  const toggleSchema = (key: string) =>
    setExpandedSchemas(prev => { const s = new Set(prev); s.has(key) ? s.delete(key) : s.add(key); return s })

  const toggleTable = (key: string) =>
    setExpandedTables(prev => { const s = new Set(prev); s.has(key) ? s.delete(key) : s.add(key); return s })

  const onDragStart = useCallback((e: React.DragEvent, table: CachedTable) => {
  ...
  return (
    <div className="text-xs">
      {Array.from(allCatalogs).sort().map(catalog => {
              ...
              <div className="ml-3">
                {catalogSchemas.map(ss => {
                  ...
                  const schemaTables = searchLower
                    ? allSchemaTables.filter(t => t.table_name.toLowerCase().includes(searchLower))
                    : allSchemaTables
                  const schExpanded = expandedSchemas.has(schKey) || (searchLower.length > 0 && schemaTables.length > 0)
                        ...
                        <div className="ml-3">
                          ...
                          {schemaTables.length === 0 && (
                            <div className={clsx(
                              ...
                              {isSyncing ? 'Fetching tables from Databricks…' : 'No tables cached yet'}
                            </div>
                          )}
                              ...
                              <div key={tKey}>
                                ...
                                >
                                  <span className="text-[#6a6a6a] w-3 shrink-0">
                                    {tExpanded ? <ChevronDown size={9} /> : <ChevronRight size={9} />}
                                  </span>
                                  <Table2 size={11} className={clsx(
                                    'shrink-0',
                                    table.table_type === 'VIEW' ? 'text-[#dcdcaa]' : 'text-[#4ec9b0]'
                                  )} />
                                  <span className="truncate text-[#cccccc]">{table.table_name}</span>
                ...
                {catalogSchemas.length === 0 && (
                  <div className="px-4 py-0.5 text-[10px] text-[#4a4a4a]">No schemas selected</div>
                )}
              </div>
            )}
          </div>
