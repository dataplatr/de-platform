...
const FN_GROUPS: {
  label: string; color: string; category: TypeCategory
  fns: { label: string; snippet: string; tip: string }[]
}[] = [
  {
    label: 'String', color: 'text-[var(--accent-fg)]', category: 'string',
    fns: [
  ...
  {
    label: 'Date', color: 'text-[var(--step-filter)]', category: 'date',
    fns: [
  ...
  {
    label: 'Number', color: 'text-[var(--step-aggregate)]', category: 'number',
    fns: [
  ...
  {
    label: 'Null', color: 'text-[var(--success)]', category: 'any',
    fns: [
...
export function TransformConfig({ nodeId, config, columns }: Props) {
  ...
  const addDerived = useCallback(() => {
    const row: TransformColumnDef = { source: '', outputName: 'new_col', castType: '', expression: '', enabled: true }
  ...
  const visibleGroups = FN_GROUPS.filter(g => g.category === 'any' || g.category === colCategory || colCategory === 'any')
            ...
            >
              ...
              <div className="flex items-center gap-1 min-w-0">
                <span className={clsx(
                  ...
                  isDerived ? 'text-[var(--step-transform)] italic' : (isSelected ? 'text-primary' : 'text-[var(--step-select)]')
                )}>
                  {isDerived ? 'derived' : row.source}
                </span>
                {isDerived
                  ? <span className="text-[9px] text-[var(--step-transform)]" title="Derived/computed column">⚡</span>
                  : colType && (
              ...
              <div className="flex items-center gap-0.5 min-w-0">
                <select
                  ...
                  className={clsx(
                    ...
                    castWarn?.level === 'error' ? 'text-error' : castWarn?.level === 'warn' ? 'text-[var(--warning)]' : 'text-muted'
                  )}
                >
        ...
        <div className="shrink-0 border-t-2 border-[var(--accent-fg)] bg-app">
          ...
          <div className="flex items-center justify-between px-3 py-1.5 border-b border-theme">
            <div className="flex items-center gap-1.5 min-w-0">
              ...
              {selectedColType && (
                <span className={clsx('text-[8px] font-bold px-1 rounded shrink-0', typeBadgeClass(selectedColType))}>
                  {selectedColType}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {selectedRow.expression && (
                <button type="button" onClick={() => update(selectedIdx!, { expression: '' })}
                  className="text-[10px] text-muted hover:text-error transition-colors" title="Clear expression">
                  Clear
                </button>
              )}
            ...
            {visibleGroups.map(group => (
              <div key={group.label} className="flex items-start gap-1.5">
                ...
                <div className="flex flex-wrap gap-0.5">
                  {group.fns.map(fn => (
                    <button key={fn.label} type="button" title={fn.tip}
