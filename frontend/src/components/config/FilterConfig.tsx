  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-[#969696] uppercase tracking-wider">Conditions</span>
        <button
          type="button"
      ...
      {config.length === 0 && (
        <p className="text-[11px] text-[#6a6a6a] italic">No conditions — all rows pass through.</p>
      )}

      <div className="flex flex-col gap-2">
        {config.map((cond, i) => (
          <div key={cond.id} className="flex flex-col gap-1 bg-[#1e1e1e] border border-[#3c3c3c] rounded p-2">
            {i > 0 && (
              <select
                value={cond.logic ?? 'AND'}
                onChange={e => update(cond.id, { logic: e.target.value as 'AND' | 'OR' })}
                className="self-start bg-[#3c3c3c] text-[#dcdcaa] text-[10px] rounded px-1 py-0.5 outline-none border-none"
              >
                <option>AND</option>
                <option>OR</option>
              </select>
            ...
            <div className="flex items-center gap-1">
              ...
              <select
                ...
                className="bg-[#2d2d30] border border-[#3c3c3c] text-[#cccccc] text-xs rounded px-1.5 py-1 outline-none"
              >
                {OPERATORS.map(op => <option key={op} value={op}>{op}</option>)}
              </select>
              {/* Delete */}
              <button type="button" onClick={() => remove(cond.id)} className="p-1 rounded hover:bg-[#3a1e1e] text-[#6a6a6a] hover:text-[#f44747] transition-colors">
                <X size={12} />
              </button>
            </div>
            {/* Value */}
            {!noValue(cond.operator) && (
              <input
                ...
                className="bg-[#2d2d30] border border-[#3c3c3c] text-[#ce9178] text-xs rounded px-2 py-1 outline-none focus:border-[#0e639c] w-full"
              />
            )}
          </div>
        ))}
