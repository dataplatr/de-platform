  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2">
        {config.map((cond, i) => (
          <div key={cond.id} className="flex flex-col gap-1.5 bg-app border border-theme rounded p-2">
            {i > 0 && (
              <select
            ...
            <div className="flex items-center gap-1 min-w-0">
              <select
                ...
                className="shrink-0 bg-elevated border border-theme text-primary text-xs rounded px-1.5 py-1 outline-none"
              >
                {OPERATORS.map((op) => (
              ...
              {!noValue(cond.operator) && (
                <input
                  ...
                  className="flex-1 min-w-0 bg-elevated border border-theme text-[var(--step-transform)] text-xs rounded px-2 py-1 outline-none focus:border-[var(--accent)]"
                />
              )}
            </div>
          </div>
