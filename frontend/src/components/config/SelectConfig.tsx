      ...
      {columns.length > 0 && (
        <>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-secondary uppercase tracking-wider flex-1">Columns</span>
            <button type="button" onClick={selectAll} className="text-[10px] text-[var(--accent-fg)] hover:underline">All</button>
            <span className="text-muted text-[10px]">/</span>
            <button type="button" onClick={clearAll} className="text-[10px] text-secondary hover:underline">None</button>
          </div>
          ...
          <div className="flex flex-col gap-1 max-h-60 overflow-y-auto scrollbar-thin">
            {columns.map(col => {
              ...
              const checked = !!sel
              return (
                <div key={col.name} className={`flex items-center gap-2 px-2 py-1 rounded transition-colors ${checked ? 'bg-[var(--selected-bg)]' : 'hover:bg-elevated'}`}>
                  <input
                    type="checkbox"
                    checked={checked}
                  ...
                  />
                  <span className={`text-xs font-mono flex-1 truncate ${checked ? 'text-[var(--step-select)]' : 'text-muted'}`}>
                    {col.name}
                  </span>
                  {checked && (
                    <input
