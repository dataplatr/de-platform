      ...
      {open && (
        <div className="absolute z-50 w-full mt-0.5 bg-surface border border-theme rounded shadow-xl overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-2 py-1 border-b border-theme">
            ...
            <span className="text-[10px] text-muted ml-auto">{selected.length}/{options.length}</span>
          </div>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <label key={opt} className={clsx(
                  ...
                  on ? 'bg-[var(--selected-bg)] hover:bg-[var(--selected-bg)]' : 'hover:bg-elevated'
                )}>
                  <input type="checkbox" checked={on} onChange={() => toggle(opt)}
                    className="accent-[var(--accent)] cursor-pointer shrink-0" />
                  <span className={clsx('text-[11px] font-mono truncate', on ? accent : 'text-secondary')}>
                    {opt}
                  </span>
                </label>
              )
