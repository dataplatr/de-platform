...
import {
  ...
  CheckSquare, Square, AlertCircle, Database, Settings, HelpCircle, Plus,
} from 'lucide-react'
...
function CatalogSchemaSearch({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api.listCatalogs(connectionId).then(async ({ data: cats }) => {
      ...
      await Promise.all(
        cats.map(async cat => {
          ...
          } catch { /* skip inaccessible catalog */ }
        })
      )
      setEntries(results)
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(e =>
      e.catalog.toLowerCase().includes(q) || e.schema.toLowerCase().includes(q)
    )
  }, [entries, query])
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        ...
        <ChevronDown size={12} className={clsx('text-[var(--text-3)] transition-transform shrink-0', open && 'rotate-180')} />
      </button>
...
function VolumeSelect({
  ...
  const loadVolumes = useCallback(() => {
    setLoading(true)
    api.listVolumes(connectionId, catalog, schema)
      .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    setVolumes([]); setCreating(false); setCreateErr(null)
    loadVolumes()
        ...
        <div className="relative">
          ...
          {showTip && (
            <div className="vol-tip absolute left-4 -top-1 z-[400] w-60 px-2.5 py-2 rounded-lg shadow-xl text-[10px] leading-relaxed">
              A <strong>Volume</strong> is a Unity Catalog folder for storing files.
              Your CSV is staged here temporarily before being loaded as a Delta table.
              The volume is only a staging area — your final table goes to <strong>{catalog}.{schema}</strong>.
            </div>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2 px-2.5 py-2.5 rounded border border-[var(--node-filter-border)] bg-[var(--node-filter-bg)]">
          <div className="flex items-start gap-2">
            ...
            <p className="text-[10px] text-[var(--warning)] leading-relaxed">
              No volumes found in <strong>{catalog}.{schema}</strong>.
        ...
        <div className="flex flex-col gap-2 px-2.5 py-2.5 rounded border border-[var(--border)] bg-[var(--bg-input)]">
          <p className="text-[10px] text-[var(--text-2)]">
            Name your staging volume in <strong>{catalog}.{schema}</strong>:
          </p>
          ...
          {createErr && (
            <p className="text-[10px] text-[var(--warning)]">{createErr}</p>
          )}
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={() => { setCreating(true); setNewVolName('csv_staging') }}
...
function LocalFileTab({
  ...
  const stageFile = useCallback((f: File) => {
    ...
    setFile(f)
    setTableName(
      f.name.replace(/\.csv$/i, '').toLowerCase().replace(/[^a-z0-9]+/g, '_').slice(0, 63)
    )
  }, [])
  ...
  const onFileInput = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (f) stageFile(f); e.target.value = ''
  }, [stageFile])

  const handleSchemaSelect = (c: string, s: string) => {
    setCatalog(c); setSchema(s); setVolume('')
  }
  ...
  const doUpload = useCallback(async () => {
    ...
    try {
      ...
      if (catalog !== conn.upload_catalog || schema !== conn.upload_schema || volume !== conn.upload_volume) {
        api.updateConnection(conn.id, {
        ...
        }).catch(() => { /* non-fatal — upload proceeds with volume passed explicitly */ })
      }
      ...
      addNode({
        id: makeNodeId(), type: 'source', label: tbl,
        tableRef: data.table_ref, connection_alias: conn.alias, sourceType: 'csv',
      ...
      api.addSchema(conn.id, catalog, schema).catch(() => {/* already added — fine */})
      api.syncSchemas(conn.id).catch(() => {/* non-fatal */})
  ...
  }, [file, conn, canUpload, catalog, schema, volume, tableName, addNode, nodes, position, onClose, pipelineConnectionAlias, setPipelineConnectionAlias])
  ...
  if (!conn) {
    return (
      <div className="flex flex-col items-center gap-3 py-8 text-center">
        ...
        <p className="text-[11px] text-[var(--text-3)]">Switch to the Databricks tab to connect first.</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <input ref={fileRef} type="file" accept=".csv" onChange={onFileInput} className="hidden" aria-label="Upload CSV file" />
      ...
      <div className="flex flex-col gap-2">
        ...
        {catalog && schema && volume && (
          <div className="flex flex-col gap-0.5">
            <label className="text-[10px] text-[var(--text-3)] uppercase font-semibold">Table Name</label>
            <input
              type="text"
              value={tableName}
            ...
            {catalog && schema && tableName && (
              <p className="text-[10px] text-[var(--text-3)] mt-0.5">
                Creates{' '}
                <span className="font-mono text-[var(--text-2)]">{catalog}.{schema}.{tableName}</span>
                {' '}as a Delta table.
              </p>
            )}
          </div>
...
function SchemaPicker({ connection, onDone }: { connection: DatabricksConnection; onDone: () => void }) {
                  ...
                  <div className="ml-4">
                    {cat.schemas.map(schema => {
                      ...
                      return (
                        ...
                        >
                          {checked
                            ? <CheckSquare size={12} className={already ? 'text-[var(--text-3)]' : 'text-[var(--success)]'} />
                            : <Square size={12} className="text-[var(--text-3)]" />}
                          <span>{schema}</span>
                          {already && <span className="ml-auto text-[9px] text-[var(--text-3)]">added</span>}
                        </div>
                      )
                    })}
                  </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={clsx(
            ...
            !adding ? 'bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)]'
                    : 'bg-[var(--bg-elevated)] text-[var(--text-3)] cursor-not-allowed'
          )}
        >
    ...
    <>
      ...
      >
        <div className="w-[480px] max-h-[90vh] overflow-y-auto bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg shadow-2xl">
          ...
          {/* Body */}
          <div className="p-4">
            {tab === 'local' && <LocalFileTab onClose={onClose} position={position} onSchemaDone={onSchemaDone} />}
              ...
              <div className="flex flex-col gap-3">
                {connections.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 py-8 text-center">
                    ...
                    <p className="text-xs text-[var(--text-2)] font-medium">No Databricks connection</p>
                  ...
                  <>
                    ...
                    {activeConn && <SchemaPicker connection={activeConn} onDone={handleSchemaDone} />}
                    <button
                      type="button"
                      onClick={() => setShowConnectModal(true)}
