...
const TOOL_DEFS: {
  ...
  { type: 'aggregate', label: 'Aggregate', Icon: BarChart3, colorClass: 'toolbar-chip-aggregate' },
  {
    ...
    label: 'Deduplicate',
    Icon: ScanLine,
    colorClass: 'toolbar-chip-deduplicate',
  },
...
function onChipDragStart(e: React.DragEvent, type: TransformNode['type'], label: string) {
  ...
  e.dataTransfer.setData('application/lakeflow-tool', JSON.stringify({ type, label }))
}
...
export function CanvasToolbar() {
  ...
  return (
    <>
      ...
      {showImport && <SourceImportModal onClose={() => setShowImport(false)} />}
    </>
  )
}
